...
public class ColetaController {
	...
	public static void Cadastro(String nomeDoador, String nomeFuncionario, String Tipo) {
		...
		if (doadores.size() <= 0 || funcionarios.size() <= 0) {
			System.out.println(
					"Não foi possivel cadastrar coleta por que nenhum doador e/ou funcionario está cadastrado!");
			return;
		}
		...
		for (ColetaDeSangue coletaExistente : listar()) {
			if (coletaExistente.getDoador().getNome().equals(nomeDoador)
					&& !(coletaExistente.getCriadoEm().plusMonths(3).getMonth() == dataAtual.getMonth())) {
	...
	public static void EstoqueTotal() {
		...
		System.out.println("A+: " + Amais + "| A-: " + Amenos + "| AB+: " + ABmais + "| AB-: " + ABmenos + "| B+: "
				+ Bmais + "| B-: " + Bmenos + "| O+: " + Omais + "| O-: " + Omenos);
	}
